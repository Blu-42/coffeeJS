...
document.addEventListener("DOMContentLoaded", function() {
    // createMenu(); //add this on your own
    // getUserLocation();
...
function getDiscount() {
    ...
    let discountNam = discountElement.value;
    let discountPerc = 1;
    if (discountNam != null && discountNam == discount.discountName) {
        discountPerc -= discount.discountAmount;
    }
...
function getWeather (latitude, longitude) {
    ...
    const weatherURL = `https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&units=imperial&appid=*****`;
    ...
    fetch(weatherURL)
        ...
        .then(data => {
            ...
            let weatherEvent = document.getElementById('weather');
            weatherEvent.innerText = `${weather}, ${temp}`;
        });
...
function hotOrCold(temp) {
    ...
    if(temp >= 70) {
        ...
        let coldItem = menu[rando].name;
        console.log ('coldItem', coldItem)  
        //suggest cold beverage
        tempElement.innerText = `Try a ${coldItem}`;
    } else {
        //suggest hot beverage
        tempElement.innerText = `It's too hot.  Try a hot beverage`;
    }
}
...
function getUserLocation () {
    ...
    navigator.geolocation.getCurrentPosition(function(position) {
        ...
        getWeather(latitude, longitude); //weather API  openweathermap.org
    }, function(error) {
...
//when getting the prompt for allowing a function to happen, if I select 'when using this website', is that creating a cookie 
    //so it know ive already selected that option the next time i go to the website